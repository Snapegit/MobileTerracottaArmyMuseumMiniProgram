	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('bowufenlei', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','bowufenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','bowufenlei');
			...
			async init(){
				...
				let res = await this.$api.info('bowufenlei', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussbowufenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
