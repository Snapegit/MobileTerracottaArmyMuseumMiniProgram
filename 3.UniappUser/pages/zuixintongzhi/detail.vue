	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('zuixintongzhi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zuixintongzhi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','zuixintongzhi');
			...
			async init(){
				...
				let res = await this.$api.info('zuixintongzhi', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discusszuixintongzhi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
