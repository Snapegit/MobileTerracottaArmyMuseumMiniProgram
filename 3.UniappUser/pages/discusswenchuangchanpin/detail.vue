...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusswenchuangchanpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusswenchuangchanpinCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusswenchuangchanpin', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusswenchuangchanpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusswenchuangchanpin');
			...
			async init(){
				...
				let res = await this.$api.info('discusswenchuangchanpin', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusswenchuangchanpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusswenchuangchanpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
