	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusswenchuangchanpin`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusswenchuangchanpin`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusswenchuangchanpin`, this.ruleForm);
						}else{
							await this.$api.add(`discusswenchuangchanpin`, this.ruleForm);
						}
						uni.setStorageSync('discusswenchuangchanpinCleanType',true);
