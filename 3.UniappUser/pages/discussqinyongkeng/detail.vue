...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussqinyongkengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussqinyongkengCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussqinyongkeng', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussqinyongkeng');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussqinyongkeng');
			...
			async init(){
				...
				let res = await this.$api.info('discussqinyongkeng', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussqinyongkeng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussqinyongkeng/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
