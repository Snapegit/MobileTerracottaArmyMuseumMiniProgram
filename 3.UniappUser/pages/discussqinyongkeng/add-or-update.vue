	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussqinyongkeng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussqinyongkeng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussqinyongkeng`, this.ruleForm);
						}else{
							await this.$api.add(`discussqinyongkeng`, this.ruleForm);
						}
						uni.setStorageSync('discussqinyongkengCleanType',true);
